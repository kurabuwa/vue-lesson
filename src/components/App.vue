...
<script>
  ...
  /*確認項目
  ・コンポーネントとは
    それぞれのソフトウェアやシステムなどの部品や要素のことである。また、webブロントエンドではページの要素を再利用可能な部品（コンポーネント）にとして分割し、それを組み合わせてページを作り上げて行く手法をコンポーネント指向という。また、コンポーネントはファイルをimportし、componentsに定義することで使用可となる。componentsに指定したキーがパスカルケースのとき、カスタムタグはvue-loaderによってケバブケースに変換される。
  ・methodsとは
  ...
  ・v-model
    form部品のinputタグやselectタグのバインディングに使われる。また、:valueでバインディングをするのと@input(inputやtextareaなどで変更が行われた際発火するイベント)の省略となっている
  ・emit
